...
def show_image(image, frame_no, x, y):
    ...
    # On-screen help (keyboard focus must be on the OpenCV window, not the terminal)
    help_lines = [
        "Keys: n=NEXT frame  p=PREV  |  > skip +36  < skip -36  |  f=first  l=last",
        "Mouse: LEFT=ball center  MIDDLE/WHEEL click=no ball  |  s=SAVE  e=EXIT",
    ]
    y0 = min(h - 50, int(h * 0.88))
    fs = max(0.45, min(0.65, w / 1200.0))
    for i, line in enumerate(help_lines):
        cv2.putText(
            image, line, (16, y0 + i * int(22 * fs + 8)),
            cv2.FONT_HERSHEY_SIMPLEX, fs, (0, 255, 255), max(1, int(2 * fs)), cv2.LINE_AA,
        )
    return image
